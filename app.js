...
saveProfileBtn.addEventListener("click", () => {
  const newName = renameInput.value.trim().slice(0, 24) || myName;
  myName = newName;
  myAvatar = popoverSelectedAvatar;
  renderMeAvatar();
  meNameEl.textContent = myName;
  if (socket) socket.emit("user:update", { username: myName, avatar: myAvatar });
  closeProfilePopover();
});
...
function connect() {
  ...
  if (!name) {
    connectError.textContent = "Введите имя";
    return;
  }

  connectError.textContent = "Подключение...";
  ...
  socket.on("connect", () => {
    myName = name;
    myAvatar = null;
    socket.emit("user:join", { username: myName, avatar: myAvatar });
